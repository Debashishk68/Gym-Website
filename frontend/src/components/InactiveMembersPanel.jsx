...
const InactiveMembersPanel = ({
  ...
  return (
    show && (
      <div className="fixed top-0 right-0 w-80 h-full bg-zinc-900 text-white z-50 shadow-lg border-l border-zinc-800 transition-transform duration-300 transform translate-x-0 overflow-y-auto">
        {/* Header */}
        ...
        <div className="p-4 border-b border-zinc-700 text-sm text-gray-400">
          <p>
            Total Inactive:{" "}
            <span className="font-semibold text-white">{inactiveMembers.length}</span>
          </p>
          <p>
            Total Due:{" "}
        ...
        <div className="p-4 space-y-3">
          {inactiveMembers.length === 0 ? (
            ...
            inactiveMembers.map((user) => (
              <div
                key={user._id}
                className="p-3 border border-zinc-800 rounded-md bg-zinc-800/50"
              >
                <p className="font-medium">{user.fullname}</p>
                <p className="text-sm text-zinc-400">{user.phone}</p>
                <p className="text-sm text-zinc-400">
                  Deadline: {new Date(user.membershipDeadline).toLocaleDateString()}
                </p>
                <p className="text-sm text-red-400 font-semibold">
                  Due: ₹{user.planPrice?.toLocaleString() || 0}
                </p>
              </div>
            ))
          )}
