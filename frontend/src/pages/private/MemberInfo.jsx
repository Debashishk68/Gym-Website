...
const MemberInfo = () => {
  ...
  //   joinedAt: "2024-08-01",
  //   membershipDeadline: "2025-08-01",
  //   status: "Paid",
  //   profilePic: "https://i.pravatar.cc/150?img=68",
  // };
  const [member,setMember ]= useState([]);
  const {memberId} = useParams();
  const { data,isSuccess,isError,error } = useMembersInfo(memberId);
  useEffect(() => {
    if(isSuccess){
      ...
      <div className="max-w-5xl mt-6 mx-auto bg-zinc-900/90 rounded-xl shadow-xl p-6 space-y-10 border border-yellow-400/20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="bg-black/60 p-5 rounded-lg border border-yellow-600/30">
            <h3 className="text-md text-yellow-400 font-medium mb-1">
              Membership Type
            </h3>
            <p className="text-white text-lg">{member.plan}</p>
          </div>
          ...
          <div
            className={`p-5 rounded-lg ${
              isExpired ? "bg-red-700" : "bg-green-700"
            } border border-yellow-500/20`}
          >
            <h3 className="text-md text-white font-medium mb-1">
              Membership Deadline
            </h3>
