...
const generateInvoicePdf = async (req, res) => {
  try {
      ...
      <html>
        <head>
          <style>
            ...
            .header img { height: 80px; filter: grayscale(100%); } /* Make logo grayscale */
        ...
        <body>
          ...
          <div class="info">
            ...
            <p><strong>To:</strong> ${invoice.name}<br>
               ${invoice.address || "Not Provided"}, ${invoice.city || "City"}, ${invoice.zip || "000000"}</p>

            <p><strong>Invoice ID:</strong> ${invoice._id?.slice(0, 8).toUpperCase() || "IN2434345"}</p>
    ...
    return res.status(500).json({ success: false, message: "Failed to generate and upload invoice" });
  }
};
